...
from sakila.film f
join sakila.film_category l on f.film_id = l.film_id
join sakila.category c on l.category_id = c.category_id
...
if(
	(year(lr.film_last_rental) = (select year(max(rental_date)) from sakila.rental))
	& (month(lr.film_last_rental) = (select month(max(rental_date)) from sakila.rental)), 
